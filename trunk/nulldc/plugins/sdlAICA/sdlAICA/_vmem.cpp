...
u8 naked fastcall _vmem_ReadMem8(u32 Address)
{
	...
	jmp [_vmem_RF8+eax];
direct:
	movzx eax,[eax+edx];	//note : upper bits dont matter , so i do 32b read here ;) (to get read of partial register stalls)
	ret;
	}
}
...
{
	printf("[arm] Read8 from 0x%X, not mapped [_vmem default handler]\n",addresss);
	return 0xD3;
}
u16 fastcall _vmem_ReadMem16_not_mapped(u32 addresss)
{
	printf("[arm] Read16 from 0x%X, not mapped [_vmem default handler]\n",addresss);
	return 0xC0D3;
}
u32 fastcall _vmem_ReadMem32_not_mapped(u32 addresss)
{
	printf("[arm] Read32 from 0x%X, not mapped [_vmem default handler]\n",addresss);
	return 0xDEADC0D3;
}
//defualt write handers
void fastcall _vmem_WriteMem8_not_mapped(u32 addresss,u8 data)
{
	printf("[arm] Write8 to 0x%X=0x%X, not mapped [_vmem default handler]\n",addresss,data);
}
void fastcall _vmem_WriteMem16_not_mapped(u32 addresss,u16 data)
{
	printf("[arm] Write16 to 0x%X=0x%X, not mapped [_vmem default handler]\n",addresss,data);
}
void fastcall _vmem_WriteMem32_not_mapped(u32 addresss,u32 data)
{
	printf("[arm] Write32 to 0x%X=0x%X, not mapped [_vmem default handler]\n",addresss,data);
}
