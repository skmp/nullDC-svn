...
void TaFifo(u32 address, u32* data, u32 size)
{
	...
	{
		u32 last_size = 
		TA_Param last_prm = PT_Reserved6;
...
u32 PvrIf::AppendStrip(VertexParam *vp)
{
	...
	if(0==idx) {
		...
		ASSERT_T((32>BytesRemain),					"AppendStrip() ByteRemaining too Low");
	}
	...
	switch(PType&0xFFFF) {
	case 0x00: { DecodeStrip0(idx, pVBuf, vp); break; }
	case 0x01: { DecodeStrip1(idx, pVBuf, vp); break; }
	case 0x02: { DecodeStrip2(idx, pVBuf, vp); break; }
	case 0x03: { DecodeStrip3(idx, pVBuf, vp); break; }
	case 0x04: { DecodeStrip4(idx, pVBuf, vp); break; }
	case 0x05: { DecodeStrip5(idx, pVBuf, vp); break; }
	case 0x06: { DecodeStrip6(idx, pVBuf, vp); break; }
	case 0x07: { DecodeStrip7(idx, pVBuf, vp); break; }
	case 0x08: { DecodeStrip8(idx, pVBuf, vp); break; }
	case 0x09: { DecodeStrip9(idx, pVBuf, vp); break; }
	case 0x0A: { DecodeStripA(idx, pVBuf, vp); break; }
	case 0x0B: { DecodeStripB(idx, pVBuf, vp); break; }
	case 0x0C: { DecodeStripC(idx, pVBuf, vp); break; }
	case 0x0D: { DecodeStripD(idx, pVBuf, vp); break; }
	case 0x0E: { DecodeStripE(idx, pVBuf, vp); break; }
	default:	ASSERT_T((1),"PvrIf::AppendStrip() Default Case Reached!"); return 0;
	}

	if(vp->pcw.EndOfStrip || (++idx>=6))
	{
		((Vertex*)pVBuf)->Size	  = idx+1;
		((Vertex*)pVBuf)->TexID	  = (u32)TCache.GetTexture(pp);
		((Vertex*)pVBuf)->ParamID = (u32)(GlobalParams.size()-1);
...
void DumpFifo(u32 address, u32* data, u32 size)
{
	...
	for(u32 i=0; i<size; i++) {
		switch( ((ParamBase*)data)[i].pcw.ParaType ) {
