...
//#define K0S_s1mpl3_c0d3
#ifdef K0S_s1mpl3_c0d3
/*
...
#define BMPR16R	16			/* EEPROM Control */
#define BMPR16	16,-1		/* EEPROM Control */
#define BMPR16_ESK	(1<<6)		/* EEPROM Shift Clocks */
#define BMPR16_ECS	(1<<5)		/* EEPROM Chip Select */
...
DLCR0_type reg_DLCR0;
DLCR1_type reg_DLCR1;
DLCR0_type reg_DLCR2;
DLCR1_type reg_DLCR3;
DLCR4_type reg_DLCR4;
DLCR5_type reg_DLCR5;
DLCR6_type reg_DLCR6;
DLCR7_type reg_DLCR7;
BMPR16_type reg_BMPR16;
BMPR17_type reg_BMPR17;
...
#define REGLOC(x)	(0xa0600400 + (x) * 4)
#define G2_8BP_RST	(0x80>>2)

/* What bank do we have selected now? */
static int la_bank = -1;
//lan_adapter: MAC address is 00:00:00:00:00:00
u8 mac_address[6]={0x00,0x00,0x1f,0x3e,0x71,0x81};

void la_reset()
{
	printf("***Reseting HIK-0300!\n");
	reg_DLCR7.ident=2;
}
u32 ReadMem_A0_006(u32 addr,u32 size)
{
	//see if its a valid reg :)
	u32 offset=(addr&0x7FF)-0x400;
	if (offset>0x3FF)
		return 0;
	offset>>=2;
	//////
#define reg_ret(name) case name##R : return reg_##name.full
	switch(offset)
	{
		reg_ret(DLCR0);
		reg_ret(DLCR1);
...
void WriteMem_A0_006(u32 addr,u32 data,u32 size)
{
	...
	case BMPR16R:
	case BMPR17R:
		break;//Ingore em , just gives wrong mac
	default:
		printf("unhandled offset=%d\n",offset);
		break;
	}
...
/*
char GASP_ID[]="GAPSPCI_BRIDGE_2";
char DATA_SHIT[0x1000];*/
//Not used in lan adapter :)
u32 ReadMem_A0_010(u32 addr,u32 size)
{
	/*
	Sleep(100);
	if (addr>=0x1001400 && addr<0x1001410)
	{
		ReadMemArrRet(GASP_ID,addr&0xF,size);
	}

	if (addr==0x1001418)
	{
		Sleep(1000);
		u32 rv = *(u32*)&DATA_SHIT[0x418];
		*(u32*)&DATA_SHIT[0x418]^=1;
		return rv;
	}

	ReadMemArrRet(DATA_SHIT,addr&0xFFF,size);
	return (rand()<<15)|rand();
	*/
	return 0;
}
...
{/*
	Sleep(1000);
	WriteMemArrRet(DATA_SHIT,addr&0xFFF,data,size);*/
}
